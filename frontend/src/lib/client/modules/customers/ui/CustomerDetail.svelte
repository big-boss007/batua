<script lang="ts">
  ...
  let {
    ...
    pointsRate?: number;
    onRefresh?: () => void;
    onWalletAction?: (action: WalletActionType, unit: WalletUnitType) => void;
  } = $props();
  ...
  function openWalletAction(actionType: WalletActionType, unitType: WalletUnitType) {
    ...
    showCashMenu = false;
    onWalletAction?.(actionType, unitType);
  }
  ...
  <section class="card">
    ...
    <div class="meta-grid meta-2">
      ...
      <div class="meta-right">
        ...
        <div class="meta-value">{formatDate(customer.created_at)}</div>
      </div>
    </div>
    {#if !showEditForm}
    ...
    <section class="card">
      ...
      <div class="wd-rows">
        <div class="wd-row">
          ...
          <div class="wd-row-right">
            <div class="wd-value-line">
              <span class="wd-row-worth"
                >worth {formatCurrencyINR(wallet.points_balance * pointsRate)}</span
              >
              <span class="wd-row-value" style="color: var(--color-primary);"
                >{formatPoints(wallet.points_balance, pointsIcon)}</span
              >
            </div>
            {#if pointsSubBuckets.length > 0}
              ...
              <div class="wd-bar">
                {#each pointsSubBuckets as b, i}
                  <div
                    class="wd-bar-seg"
                    style="flex: {Math.round(b.spendable)}; background: var(--color-points-{i});"
                  ></div>
                {/each}
              </div>
              <div class="wd-legend">
                {#each pointsSubBuckets as b, i}
                  <span class="wd-legend-item"
                    ><span class="wd-legend-dot" style="background: var(--color-points-{i});"
                    ></span>{bucketLabel(b.bucket_type)}
                    <span class="wd-legend-val">{Math.round(b.spendable)}</span></span
                  >
                {/each}
              </div>
            {/if}
            ...
            <div class="wd-menu-wrap">
              ...
              {#if showPointsMenu}
                <div class="wd-dropdown">
                  <button
                    class="wd-dropdown-item wd-dropdown-green"
                    onclick={() => openWalletAction('add', 'points')}>Add</button
                  >
                  {#if wallet.points_balance > 0}
                    <button
                      class="wd-dropdown-item wd-dropdown-red"
                      onclick={() => openWalletAction('remove', 'points')}>Remove</button
                    >
                    <button
                      class="wd-dropdown-item wd-dropdown-amber"
                      onclick={() => openWalletAction('expire', 'points')}>Expire</button
                    >
                  {/if}
                </div>
              {/if}
        ...
        <div class="wd-row">
          ...
          <div class="wd-row-right">
            <div class="wd-value-line">
              <span class="wd-row-value" style="color: var(--color-success);"
                >{formatCurrencyINR(wallet.cash_balance)}</span
              >
            </div>
            {#if pointsCashBuckets.length > 0}
              <div class="wd-bar">
                {#each pointsCashBuckets as b, i}
                  <div
                    class="wd-bar-seg"
                    style="flex: {Math.round(b.spendable)}; background: var(--color-cash-{i});"
                  ></div>
                {/each}
              </div>
              <div class="wd-legend">
                {#each pointsCashBuckets as b, i}
                  <span class="wd-legend-item"
                    ><span class="wd-legend-dot" style="background: var(--color-cash-{i});"
                    ></span>{bucketLabel(b.bucket_type)}
                    <span class="wd-legend-val">{formatCurrencyINR(b.spendable)}</span></span
                  >
                {/each}
              </div>
            {:else}
            ...
            <div class="wd-menu-wrap">
              ...
              {#if showCashMenu}
                <div class="wd-dropdown">
                  <button
                    class="wd-dropdown-item wd-dropdown-green"
                    onclick={() => openWalletAction('add', 'cash')}>Add</button
                  >
                  {#if wallet.cash_balance > 0}
                    <button
                      class="wd-dropdown-item wd-dropdown-red"
                      onclick={() => openWalletAction('remove', 'cash')}>Remove</button
                    >
                    <button
                      class="wd-dropdown-item wd-dropdown-amber"
                      onclick={() => openWalletAction('expire', 'cash')}>Expire</button
                    >
                  {/if}
                </div>
              {/if}
    ...
    {#if tier}
      <div class="tier-row">
        <span class="tier-badge"
          >{tier.tier_name} <span class="tier-mult">{tier.earn_rate_multiplier}x</span></span
        >
      ...
      {#if membershipHigher}
        <div class="callout callout-upgrade">
          ...
          <span
            >Membership active — currently earning at <strong>{membershipMult}x</strong>
            ({membership?.tier_name}) instead of {earnedMult}x ({tier.tier_name})</span
          >
        </div>
      {:else if earnedHigher}
        <div class="callout callout-muted">
          <span class="callout-icon"><Icon name="crown" size={16} /></span>
          <span
            >Earned tier is higher — earning at <strong>{earnedMult}x</strong> (membership {membership?.tier_name}
            {membershipMult}x is superseded)</span
          >
        </div>
      {:else if sameTier}
        <div class="callout callout-muted">
    ...
    {#if membership}
      <div class="tier-row">
        <span class="tier-badge"
          ...
          <span class="tier-mult">{membership.earn_rate_multiplier}x</span></span
        >
        {#if membershipExpiring}
      ...
      <div class="progress">
        <div class="progress-header">
          ...
          <span class="progress-value" class:progress-value-warn={membershipExpiring}
            >{membership.days_remaining} days</span
          >
        </div>
        <Progress
          value={Math.min(100, (membership.days_remaining / 365) * 100)}
          classes={membershipExpiring ? 'progress-bar-amber' : 'progress-bar-green'}
        />
      </div>
    {:else}
      ...
      <div class="meta-grid meta-3">
        {#if referral.code !== null}
          <div>
            ...
            <div class="meta-value meta-mono">{referral.code}</div>
          </div>
        {/if}
        <div>
          <span class="meta-label">Referrals made</span>
          <div class="meta-value meta-highlight">{referral.referrals_made}</div>
        </div>
        <div>
          <span class="meta-label">Rewards earned</span>
          <div class="meta-value">{formatCurrencyINR(referral.rewards_earned)}</div>
        </div>
      </div>
    {:else}
...
<style>
  ...
  .wd-row {
    --color-points-0: var(--purple-500);
    --color-points-1: var(--purple-500);
    --color-points-2: color-mix(in srgb, var(--purple-500) 30%, #fff);
    --color-points-3: color-mix(in srgb, var(--purple-500) 18%, #fff);
    --color-points-4: color-mix(in srgb, var(--purple-500) 12%, #fff);
  }
  .wd-row {
    --color-cash-0: var(--green-500);
    --color-cash-1: var(--green-100);
    --color-cash-2: var(--green-100);
    --color-cash-3: var(--green-100);
    --color-cash-4: var(--green-100);
  }
  ...
  :global(.progress-bar-purple) {
    --progress-fill-color: var(--color-primary);
    --progress-height: 6px;
    --progress-border-radius: 3px;
    --progress-background-color: var(--color-border);
  }
  :global(.progress-bar-green) {
    --progress-fill-color: var(--color-success);
    --progress-height: 6px;
    --progress-border-radius: 3px;
    --progress-background-color: var(--color-border);
  }
  :global(.progress-bar-amber) {
